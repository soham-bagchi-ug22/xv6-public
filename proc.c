...
struct {
  ...
  struct proc * queueHead[MAXPRIORITY]; //trying to implement the LL queue structures within ptable
...
enqueue(struct proc *p, int priority){
  p->priority = priority;
    ...
    p->next = (struct proc *) 0;
    //cprintf("%p\n", p);
		ptable.queueTail[priority]->next = p;
		ptable.queueTail[priority] = p;
...
dequeue(int priority){
    ...
    struct proc *retProc = ptable.queueHead[priority];
    ...
    if(ptable.queueHead[priority] == ptable.queueTail[priority]){
      //cprintf("%p\n", ptable.queueHead[priority]->next);
      ptable.queueHead[priority] = (struct proc *) 0;
      ptable.queueTail[priority] = (struct proc *) 0;
    }
		    
    else {
      ptable.queueHead[priority] = retProc->next;
      //cprintf("%p %p\n", ptable.queueHead[priority], ptable.queueTail[priority]);
    }
    //cprintf("RP %p\n", retProc);
		return retProc;
	}
...
//Shifted syscall functions to top to avoid possible "undefined" errors
